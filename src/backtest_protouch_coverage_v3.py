...
def select(P,maxb=128):
 lp,C=beam(P); S=scenarios(P)
 # Store only threshold masks. This avoids repeatedly constructing a candidates x scenarios utility matrix.
 H=np.empty((len(C),len(S)),np.uint8)
 chunk=250
 for a in range(0,len(C),chunk):
  H[a:a+chunk]=(C[a:a+chunk,None,:]==S[None,:,:]).sum(2)
 M11=H>=11; M12=H>=12; M13=H>=13; del H
 cov11=np.zeros(len(S),bool);cov12=np.zeros(len(S),bool);cov13=np.zeros(len(S),bool)
 alive=np.ones(len(C),bool); chosen=[]
 for _ in range(min(maxb,len(C))):
  gain=(M11[:,~cov11].sum(1)*W11 + M12[:,~cov12].sum(1)*W12 + M13[:,~cov13].sum(1)*W13)
  gain[~alive]=-1; mx=gain.max(); ix=np.flatnonzero(gain==mx); i=int(ix[np.argmax(lp[ix])])
  chosen.append(C[i]);alive[i]=False;cov11|=M11[i];cov12|=M12[i];cov13|=M13[i]
...
out={"version":"0.4.1","optimizer":"multi-objective scenario coverage optimized","weights":{"11":W11,"12":W12,"13":W13},"seed":42,"simulations_per_pool":12000,"dataset":"2025 untouched OOS predictions","results":res}
