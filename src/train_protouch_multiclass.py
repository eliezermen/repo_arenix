...
pv_l=logit(tr,va); pv_x=xgb(tr,va); pt_l=logit(fit,te); pt_x=xgb(fit,te)
...
w=best[1]; pv=w*pv_l+(1-w)*pv_x; pt=w*pt_l+(1-w)*pt_x
...
Path("artifacts/protouch_multiclass_0.1.0.json").write_text(json.dumps(meta,indent=2)); print(json.dumps(meta,indent=2))
